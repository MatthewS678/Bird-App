...
def user_stats(path=None):
    ...
    query = (db.sightings.sampling_event == db.checklists.sampling_event) & (db.checklists.observer_id == user_email)
    ...
    grid = Grid(path,
        ...
        search_queries=[ # allows for better searching with specific filters 
            ...
            ['Search by Latitude', lambda val: (db.checklists.latitude >= float(val) - 10) &
                                                  (db.checklists.latitude <= float(val) + 10)],
            ['Search by Longitude', lambda val: (db.checklists.longitude >= float(val) - 10) &
                                                   (db.checklists.longitude <= float(val) + 10)],
        ],
        fields=[
